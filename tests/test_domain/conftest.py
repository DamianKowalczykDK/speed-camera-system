...
def clear_database(connection_manager: MySQLConnectionManager) -> None:
    with connection_manager.get_connection() as conn, conn.cursor() as cursor:
        ...
        for (table_name,) in tables:
            table_name_str = (
                table_name.decode() if isinstance(table_name, bytes) else table_name
            )
...
def mock_connection_manager() -> MagicMock:
    return MagicMock(spec=MySQLConnectionManager)


@pytest.fixture
def mock_violation_repository_with_mocked_query(
    mock_connection_manager: MagicMock,
) -> ViolationRepository:
    return ViolationRepository(mock_connection_manager)
