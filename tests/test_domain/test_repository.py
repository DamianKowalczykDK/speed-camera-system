...
def test_insert_offense(
    offense_repository: OffenseRepository, offense_1: Offense
) -> None:
...
def test_insert_driver_and_find_all(
    driver_repository: OffenseRepository, driver_1: Driver, clear_database
) -> None:
    driver_id = driver_repository.insert(driver_1)
    assert driver_id is not None
...
def test_update_driver(
    driver_repository: DriverRepository, driver_1: Driver, driver_2: Driver
) -> None:
    driver_id = driver_repository.insert(driver_1)
...
def test_insert_many_if_not_rows(
    driver_repository: DriverRepository, clear_database
) -> None:
...
def test_execute_sql_file(connection_manager: MySQLConnectionManager) -> None:
    ...
    with pytest.raises(Error):
        execute_sql.execute_sql_file(
            os.path.join(os.path.dirname(__file__), "../../sql/data_test.sql")
        )
...
def test_get_most_popular_speed_camera(
    ...
    with patch.object(
        ...
        result = (
            mock_violation_repository_with_mocked_query.get_most_popular_speed_camera()
        )
...
def test_get_driver_points(
    ...
    with patch.object(
        ViolationRepository, "_execute_query", return_value=[top_driver_data_dict_1]
    ) as mock_execute_query:
...
def test_find_violations_with_offense_by_driver(
    ...
    with patch.object(
        ViolationRepository, "_execute_query", return_value=[driver_offense_data_dict_1]
    ) as mock_execute_query:
        result = mock_violation_repository_with_mocked_query.find_violations_with_offense_by_driver(
            "K123456"
        )
        assert mock_execute_query.call_count == 1
    assert result[0]["first_name"] == "John"
...
def test_execute_query_if_not_data(
    violation_repository: ViolationRepository, clear_database
) -> None:
